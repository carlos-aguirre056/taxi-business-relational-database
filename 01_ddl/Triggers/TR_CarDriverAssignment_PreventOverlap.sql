...
BEGIN
    ...
    -- Prevent overlapping assignments for the same car
    IF EXISTS
    (
        SELECT 1
        FROM inserted AS i
        INNER JOIN CarDriverAssignment AS c
           ...
           AND c.id_car_driver_assignment <> i.id_car_driver_assignment
           AND i.start_date < COALESCE(c.end_date, '9999-12-31 23:59:59')
           AND c.start_date < COALESCE(i.end_date, '9999-12-31 23:59:59')
    )
    BEGIN
        THROW 50001 --custom error number assigned to car overlap ,'Temporal integrity violation: overlapping driver assignments exist for the same car.', 1;
    END;
    ...
    BEGIN
        THROW 50002 --custom error number assigned to car overlap , 'Temporal integrity violation: overlapping car assignments exist for the same driver.', 1;
    END;
END;
GO
